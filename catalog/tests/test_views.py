...
class AuthorListViewTest(TestCase):
    ...
    def test_lists_all_authors(self):
        # Get second page and confirm it have (exactly) remaining 8 items
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_form_invalid_renewal_date_past(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        data_in_past = datetime.date.today() - datetime.timedelta(weeks=1)
        response = self.client.post(
            reverse('renew-book-librarian', kwargs={'pk': self.test_bookinstance1.pk}),
            {'due_back': data_in_past}
        )
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'form', 'due_back', 'Invalid date - renewal in pas')
